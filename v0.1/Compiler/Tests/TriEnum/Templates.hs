...
-}
module Compiler.Tests.TriEnum.Templates where
...
-- |mirrVMap template
t99 :: Monad m => Template m
t99 (Lf (LfTy "DMap" [mode1, kt1, vt1, ordF1, parF1, parD1, mirD1]) :-> 
     Lf (LfTy "DMap" [mode2, kt2, vt2, ordF2, parF2, parD2, mirD2]))
   (LFun _ inN outN)
   | match (namedTy "Vec") [mode1, mode2] &&
     kt1 == kt2 && vt1 == vt2 = do
     -- check in and out dists are same
     assertM (return $ parD1 == parD2) "par dims don't match"
     -- get input var name 
