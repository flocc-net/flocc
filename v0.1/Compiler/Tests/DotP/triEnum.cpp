...
inline bool operator()(const struct7099& a, const struct7099& b) const {
 ...
 // user def reduction function
  void userFun7170(const void* invec, void *inoutvec, int len, const MPI::Datatype& datatype) {
...
for (int v2 = 0; v2 < 1024*1024/Nproc; v2++) {
  ...
  // END mapDStream2 consume
    ...
    v7054.finish();
    // DBG dStreamToDVecMap.count
    #ifdef DEBUG
    std::cerr << thisRank << ") DBG dStreamToDVecMap.count = " << v7054.getConsumer()->size() << "\n";
    #endif
...
if (true) {
// DBG sortDVecMap.beforeCount
#ifdef DEBUG
std::cerr << thisRank << ") DBG sortDVecMap.beforeCount = " << v7053->size() << "\n";
#endif

v7074->assign(v7053);// DBG sortDVecMap.afterCount
#ifdef DEBUG
std::cerr << thisRank << ") DBG sortDVecMap.afterCount = " << v7074->size() << "\n";
#endif
...
for (; v7131 != v7132; v7131++) {
  ...
  // END eqJoinDStreamDVecMap consume
      ...
      if (true) {
        cartComm.Allreduce(&v7159, &v7156, sizeof(int), MPI_PACKED, v7175);
      }
