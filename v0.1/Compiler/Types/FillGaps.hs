...
-}
module Compiler.Types.FillGaps (possibleSubs, remRedundantDims) where
...
import Compiler.Types.EmbeddedFunctions (getIdxTreeTy, getVarTreeTy, getSimpleFunTy)
import qualified Compiler.Types.Substitutions as S

import Control.Monad.State.Strict (StateT, modify, lift, runStateT)
import Data.Functor.Identity (runIdentity)
import Data.List (subsequences, permutations, intercalate)
...
getRedundantDims :: [TyTerm] -> [TyTerm]
getRedundantDims types = map Var $ DS.toList redVars
...
remRedundantDims types = IM.map (runIdentity . S.applySubstsMap subs) types 
  where redVars = getRedundantDims $ IM.elems types
...
possibleFuns funKind env domTy ranTy funT = case funT of
  -- fun is just a var
  (Var vid) -> do
  ...
  (Term (Tok (Ty "GFst")) [f]) -> case domTy of
  ...
  (Term (Tok (Ty "GSnd")) [f]) -> case domTy of
...
possibleAppFuns env exp = case exp of
  -- fun application
...
termToVarTree term = case term of
  (Term TupTok l) -> do
...
applyVarSubs exp l1 l2 = applyVarSubsts subs exp
  where subL = map (\(Var vid,t2) -> (vid, t2)) $ zip l1 l2
