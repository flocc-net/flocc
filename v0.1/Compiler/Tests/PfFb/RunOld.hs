...
import Compiler.Types2.TypeAssignment (assignTypes, showExprWithTypes, TyToken(Ty))
import Compiler.Types2.DepTypeAssignment (assignDepTypes, showExprWithDepTypes)
import Compiler.Types2.EmbeddedFunctions (simplifyFunsInEnv, fullySimplifyFun)
...
import Compiler.Tests.PfFb.Templates
...
main2 :: IdxMonad IO ()
main2 = do
  lift $ putStrE "PerformanceFeedback code gen from solution source:\n"  
  ...
  -- load source file
  let testFile = curDir ++ relDir ++ "histMap1.flocc" --"test1.flocc"
  ...
  lift $ putStrE $ showExprTreeWithIds ast' --show ast'
  lift $ putStrE "\nSuccess.\n\n"

  --lift $ putStr $ show $ length ${- $ intercalate "\n" $ map show $-} filterDups $ takeWhile isJust castIdxs
  ...
  -- find casts to make types unify
  lift $ putStr "Searching for casts...\n"
  sols <- findCasts (IM.fromList typeDefs) (Data.Map.fromList varIds) ast'
  case sols of
    [] -> error "no cast solutions found!\n"
    ((h,_,_):r) -> do
      -- show solutions
      lift $ putStr $ concat $ map (\(e,_,c) -> (show c) ++ ": " ++ (show e) ++ "\n") $ take 10 sols
      lift $ putStr $ "first cast solution:\n" ++ (show h)
      -- check types for the first solution
      lift $ putStrE "Inferring program types...\n"
      astTypes <- assignDepTypes (IM.fromList typeDefs) h
      lift $ putStrE "Success.\n"
      lift $ putStrE $ showExprWithDepTypes astTypes h
      lift $ putStr "\n"
